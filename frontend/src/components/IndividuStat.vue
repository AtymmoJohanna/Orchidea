<template>
  <div class="stat-container">
    <h2>Statistiques des orchidées par nombre d'individus</h2>
...
const fetchOrchidees = async () => {
  try {
    ...
    // Comptage par nbreIndividu
    const countByNbreIndividu = orchidées.value.reduce((acc, orchidee) => {
      const nbreIndividu = orchidee.nbreIndividu || "Inconnu"; // Gérer les nbreIndividu null/undefined
      acc[nbreIndividu] = (acc[nbreIndividu] || 0) + 1;
      return acc;
    }, {});

    console.log("Comptage par nombre d'individus : ", countByNbreIndividu);

    // Mettre à jour les séries et les catégories
    const nbreIndividus = Object.keys(countByNbreIndividu); // nbreIndividu pour l'axe X
    const counts = Object.values(countByNbreIndividu); // Nombres pour l'axe Y

    console.log("Nombres d'individus (axe X) : ", nbreIndividus);
